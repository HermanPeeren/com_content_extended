...
class FormModel extends \Yepr\Component\Content_extended\Administrator\Model\ArticleModel
{
     ...
     * @var        string
     */
    public $typeAlias = 'com_content_extended.article';
    ...
    public function getItem($itemId = null)
    {
        ...
        $userId = $user->id;
        $asset  = 'com_content_extended.article.' . $value->id;
        ...
        if ($itemId) {
            ...
            if ($catId) {
                $value->params->set('access-change', $user->authorise('core.edit.state', 'com_content_extended.category.' . $catId));
                $value->catid = $catId;
            } else {
                $value->params->set('access-change', $user->authorise('core.edit.state', 'com_content_extended'));
            }
        }
        ...
        if ($itemId) {
            ...
            $value->tags->getTagIds($value->id, 'com_content_extended.article');
    ...
    public function save($data)
    {
        ...
        if (
            ...
            && $associations = Associations::getAssociations('com_content_extended', '#__content', 'com_content_extended.item', $data['id'])
        ) {
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        if ($id > 0 && !$user->authorise('core.edit.state', 'com_content_extended.article.' . $id)) {
        ...
        if ($this->getState('article.id') && Associations::isEnabled()) {
            $associations = Associations::getAssociations('com_content_extended', '#__content', 'com_content_extended.item', $id);
