...
class ArticlesModel extends ListModel
{
    ...
    protected function populateState($ordering = 'ordering', $direction = 'ASC')
    {
        ...
        if ((!$params->get('show_noauth')) || (!ComponentHelper::getParams('com_content_extended')->get('show_noauth'))) {
    ...
    protected function getListQuery()
    {
        ...
        if (\is_array($tagId)) {
                ...
                $subQuery = $db->getQuery(true)
                    ...
                    ->where(
                        [
                            ...
                            $db->quoteName('type_alias') . ' = ' . $db->quote('com_content_extended.article'),
                        ]
                    );
            ...
            $query->join(
                ...
                $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
                    . ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_content_extended.article')
            )
    ...
    public function getItems()
    {
        ...
        $globalParams = ComponentHelper::getParams('com_content_extended', true);
        ...
        foreach ($items as $item) {
             ...
             */
            if (!$guest) {
                $asset = 'com_content_extended.article.' . $item->id;
        ...
        if ($taggedItems) {
            ...
            foreach ($tagsHelper->getMultipleItemTags('com_content_extended.article', $itemIds) as $id => $tags) {
