...
class HtmlView extends CategoryView
{
     ...
     * @since  3.2
     */
    protected $extension = 'com_contentextended';
    ...
    public function display($tpl = null)
    {
        ...
        foreach ($this->items as $item) {
            ...
            $app->triggerEvent('onContentPrepare', ['com_contentextended.category', &$item, &$item->params, 0]);

            // Old plugins: Use processed text as introtext
            $item->introtext = $item->text;

            $results                        = $app->triggerEvent('onContentAfterTitle', ['com_contentextended.category', &$item, &$item->params, 0]);
            $item->event->afterDisplayTitle = trim(implode("\n", $results));

            $results                           = $app->triggerEvent('onContentBeforeDisplay', ['com_contentextended.category', &$item, &$item->params, 0]);
            $item->event->beforeDisplayContent = trim(implode("\n", $results));

            $results                          = $app->triggerEvent('onContentAfterDisplay', ['com_contentextended.category', &$item, &$item->params, 0]);
    ...
    protected function prepareDocument()
    {
        ...
        if (
            $menu && $menu->component == 'com_contentextended' && isset($menu->query['view'])
