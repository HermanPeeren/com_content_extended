...
class ArticlesModel extends ListModel
{
    ...
    protected function populateState($ordering = 'ordering', $direction = 'ASC')
    {
        ...
        if ((!$user->authorise('core.edit.state', 'com_contentextended')) && (!$user->authorise('core.edit', 'com_contentextended'))) {
        ...
        if ((!$params->get('show_noauth')) || (!ComponentHelper::getParams('com_contentextended')->get('show_noauth'))) {
    ...
    protected function getListQuery()
    {
        ...
        if (\is_array($tagId)) {
                ...
                $subQuery = $db->getQuery(true)
                    ...
                    ->where(
                        [
                            ...
                            $db->quoteName('type_alias') . ' = ' . $db->quote('com_contentextended.article'),
                        ]
                    );
            ...
            $query->join(
                ...
                $db->quoteName('tagmap.content_item_id') . ' = ' . $db->quoteName('a.id')
                    . ' AND ' . $db->quoteName('tagmap.type_alias') . ' = ' . $db->quote('com_contentextended.article')
            )
    ...
    public function getItems()
    {
        ...
        $globalParams = ComponentHelper::getParams('com_contentextended', true);
        ...
        foreach ($items as $item) {
             ...
             */
            if (!$guest) {
                $asset = 'com_contentextended.article.' . $item->id;
        ...
        if ($taggedItems) {
            ...
            foreach ($tagsHelper->getMultipleItemTags('com_contentextended.article', $itemIds) as $id => $tags) {
