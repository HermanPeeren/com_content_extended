...
class FormModel extends \Yepr\Component\ContentExtended\Administrator\Model\ArticleModel
{
     ...
     * @var        string
     */
    public $typeAlias = 'com_contentextended.article';
    ...
    public function getItem($itemId = null)
    {
        ...
        $userId = $user->id;
        $asset  = 'com_contentextended.article.' . $value->id;
        ...
        if ($itemId) {
            ...
            if ($catId) {
                $value->params->set('access-change', $user->authorise('core.edit.state', 'com_contentextended.category.' . $catId));
                $value->catid = $catId;
            } else {
                $value->params->set('access-change', $user->authorise('core.edit.state', 'com_contentextended'));
            }
        }
        ...
        if ($itemId) {
            ...
            $value->tags->getTagIds($value->id, 'com_contentextended.article');
    ...
    public function save($data)
    {
        ...
        if (
            ...
            && $associations = Associations::getAssociations('com_contentextended', '#__content', 'com_contentextended.item', $data['id'])
        ) {
    ...
    public function getForm($data = [], $loadData = true)
    {
        ...
        if ($id > 0 && !$user->authorise('core.edit.state', 'com_contentextended.article.' . $id)) {
        ...
        if ($this->getState('article.id') && Associations::isEnabled()) {
            $associations = Associations::getAssociations('com_contentextended', '#__content', 'com_contentextended.item', $id);
