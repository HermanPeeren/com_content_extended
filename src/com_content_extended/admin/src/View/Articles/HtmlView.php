...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        $this->hits          = ComponentHelper::getParams('com_content_extended')->get('record_hits', 1) == 1;

        if (!\count($this->items) && $this->isEmptyState = $this->get('IsEmptyState')) {
            $this->setLayout('emptystate');
        }

        if (ComponentHelper::getParams('com_content_extended')->get('workflow_enabled')) {
    ...
    {
        $canDo   = ContentHelper::getActions('com_content_extended', 'category', $this->state->get('filter.category_id'));
        $user    = $this->getCurrentUser();
        $toolbar = $this->getDocument()->getToolbar();

        ToolbarHelper::title(Text::_('COM_CONTENT_ARTICLES_TITLE'), 'copy article');

        if ($canDo->get('core.create') || \count($user->getAuthorisedCategories('com_content_extended', 'core.create')) > 0) {
        ...
        if ($user->authorise('core.admin', 'com_content_extended') || $user->authorise('core.options', 'com_content_extended')) {
            $toolbar->preferences('com_content_extended');
        }
