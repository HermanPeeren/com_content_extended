...
class CategoryModel extends ListModel
{
     ...
     * @var  string
     */
    protected $_context = 'com_content_extended.category';
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $value = $this->getUserStateFromRequest('com_content_extended.category.filter.' . $itemid . '.tag', 'filter_tag', 0, 'int', false);
        $this->setState('filter.tag', $value);

        // Optional filter text
        $search = $app->getUserStateFromRequest('com_content_extended.category.list.' . $itemid . '.filter-search', 'filter-search', '', 'string');
        $this->setState('list.filter', $search);

        // Filter.order
        $orderCol = $app->getUserStateFromRequest('com_content_extended.category.list.' . $itemid . '.filter_order', 'filter_order', '', 'string');
        ...
        $listOrder = $app->getUserStateFromRequest('com_content_extended.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
        ...
        if (($app->getInput()->get('layout') === 'blog') || $params->get('layout_type') === 'blog') {
            ...
            $limit = $app->getUserStateFromRequest('com_content_extended.category.list.' . $itemid . '.limit', 'limit', $params->get('display_num'), 'uint');
        }
    ...
    public function getItems()
    {
        ...
        if ($this->_articles === null && $category = $this->getCategory()) {
            $model = $this->bootComponent('com_content_extended')->getMVCFactory()
    ...
    protected function _buildContentOrderBy()
    {
        ...
        $itemid    = $app->getInput()->get('id', 0, 'int') . ':' . $app->getInput()->get('Itemid', 0, 'int');
        $orderCol  = $app->getUserStateFromRequest('com_content_extended.category.list.' . $itemid . '.filter_order', 'filter_order', '', 'string');
        $orderDirn = $app->getUserStateFromRequest('com_content_extended.category.list.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', '', 'cmd');
        $orderby   = ' ';
    ...
    public function getCategory()
    {
        if (!\is_object($this->_item)) {
            ...
            if (\is_object($this->_item)) {
                $user  = $this->getCurrentUser();
                $asset = 'com_content_extended.category.' . $this->_item->id;
