...
class HtmlView extends CategoryView
{
    ...
    public function display($tpl = null)
    {
        ...
        foreach ($this->items as $item) {
            ...
            $app->triggerEvent('onContentPrepare', ['com_content_extended.category', &$item, &$item->params, 0]);

            // Old plugins: Use processed text as introtext
            $item->introtext = $item->text;

            $results                        = $app->triggerEvent('onContentAfterTitle', ['com_content_extended.category', &$item, &$item->params, 0]);
            $item->event->afterDisplayTitle = trim(implode("\n", $results));

            $results                           = $app->triggerEvent('onContentBeforeDisplay', ['com_content_extended.category', &$item, &$item->params, 0]);
            $item->event->beforeDisplayContent = trim(implode("\n", $results));

            $results                          = $app->triggerEvent('onContentAfterDisplay', ['com_content_extended.category', &$item, &$item->params, 0]);
    ...
    protected function prepareDocument()
    {
        ...
        if (
            $menu && $menu->component == 'com_content_extended' && isset($menu->query['view'])
