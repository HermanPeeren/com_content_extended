...
if ($saveOrder && !empty($this->items)) {
    $saveOrderingUrl = 'index.php?option=com_contentextended&task=articles.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
    HTMLHelper::_('draggablelist.draggable');
}

$workflow_enabled  = ComponentHelper::getParams('com_contentextended')->get('workflow_enabled');
...
if ($workflow_enabled) :
    ...
    ->useScript('com_contentextended.articles-status');

    $workflow_state    = Factory::getApplication()->bootComponent('com_contentextended')->isFunctionalityUsed('core.state', 'com_contentextended.article');
    $workflow_featured = Factory::getApplication()->bootComponent('com_contentextended')->isFunctionalityUsed('core.featured', 'com_contentextended.article');
endif;

$assoc = Associations::isEnabled();
?>

<form action="<?php echo Route::_('index.php?option=com_contentextended&view=articles'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="row">
        <div class="col-md-12">
                ...
                <?php else : ?>
                    <table class="table itemList" id="articleList">
                        ...
                        <?php foreach ($this->items as $i => $item) :
                            $item->max_ordering = 0;
                            $canEdit              = $user->authorise('core.edit', 'com_contentextended.article.' . $item->id);
                            $canCheckin           = $user->authorise('core.manage', 'com_checkin') || $item->checked_out == $userId || is_null($item->checked_out);
                            $canEditOwn           = $user->authorise('core.edit.own', 'com_contentextended.article.' . $item->id) && $item->created_by == $userId;
                            $canChange            = $user->authorise('core.edit.state', 'com_contentextended.article.' . $item->id) && $canCheckin;
                            $canExecuteTransition = $user->authorise('core.execute.transition', 'com_contentextended.article.' . $item->id);
                            $canEditCat           = $user->authorise('core.edit', 'com_contentextended.category.' . $item->catid);
                            $canEditOwnCat        = $user->authorise('core.edit.own', 'com_contentextended.category.' . $item->catid) && $item->category_uid == $userId;
                            $canEditParCat        = $user->authorise('core.edit', 'com_contentextended.category.' . $item->parent_category_id);
                            $canEditOwnParCat     = $user->authorise('core.edit.own', 'com_contentextended.category.' . $item->parent_category_id) && $item->parent_category_uid == $userId;
                                ...
                                <th scope="row" class="has-context">
                                    <div class="break-word">
                                        ...
                                        <?php if ($canEdit || $canEditOwn) : ?>
                                            <a href="<?php echo Route::_('index.php?option=com_contentextended&task=article.edit&id=' . $item->id); ?>" title="<?php echo Text::_('JACTION_EDIT'); ?> <?php echo $this->escape($item->title); ?>">
                                        ...
                                        <div class="small">
                                            <?php
                                            $ParentCatUrl = Route::_('index.php?option=com_categories&task=category.edit&id=' . $item->parent_category_id . '&extension=com_contentextended');
                                            $CurrentCatUrl = Route::_('index.php?option=com_categories&task=category.edit&id=' . $item->catid . '&extension=com_contentextended');
                    ...
                    <?php
                    if (
                        $user->authorise('core.create', 'com_contentextended')
                        && $user->authorise('core.edit', 'com_contentextended')
                        && $user->authorise('core.edit.state', 'com_contentextended')
                    ) : ?>
