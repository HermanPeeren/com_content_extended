...
class ContentHelper extends \Joomla\CMS\Helper\ContentHelper
{
    ...
    {
        if ($form->getName() != 'com_categories.categorycom_contentextended') {
            return;
        }
        ...
        $component = Factory::getApplication()->bootComponent('com_contentextended');

        if (
            !$component instanceof WorkflowServiceInterface
            || !$component->isWorkflowActive('com_contentextended.article')
        ) {
        ...
        $query->select($db->quoteName('title'))
            ...
            ->where(
                [
                    ...
                    $db->quoteName('extension') . ' = ' . $db->quote('com_contentextended.article'),
                ]
            );
