...
class ContentHelper extends \Joomla\CMS\Helper\ContentHelper
{
    ...
    {
        if ($form->getName() != 'com_categories.categorycom_content_extended') {
            return;
        }
        ...
        $component = Factory::getApplication()->bootComponent('com_content_extended');

        if (
            !$component instanceof WorkflowServiceInterface
            || !$component->isWorkflowActive('com_content_extended.article')
        ) {
        ...
        $query->select($db->quoteName('title'))
            ...
            ->where(
                [
                    ...
                    $db->quoteName('extension') . ' = ' . $db->quote('com_content_extended.article'),
                ]
            );
