...
if ($saveOrder && !empty($this->items)) {
    $saveOrderingUrl = 'index.php?option=com_content_extended&task=articles.saveOrderAjax&tmpl=component&' . Session::getFormToken() . '=1';
    HTMLHelper::_('draggablelist.draggable');
}

$workflow_enabled  = ComponentHelper::getParams('com_content_extended')->get('workflow_enabled');
...
if ($workflow_enabled) :
    ...
    ->useScript('com_content_extended.articles-status');

    $workflow_state    = Factory::getApplication()->bootComponent('com_content_extended')->isFunctionalityUsed('core.state', 'com_content_extended.article');
    $workflow_featured = Factory::getApplication()->bootComponent('com_content_extended')->isFunctionalityUsed('core.featured', 'com_content_extended.article');
endif;

$assoc = Associations::isEnabled();
?>

<form action="<?php echo Route::_('index.php?option=com_content_extended&view=articles'); ?>" method="post" name="adminForm" id="adminForm">
    <div class="row">
        <div class="col-md-12">
                ...
                <?php else : ?>
                    <table class="table itemList" id="articleList">
                        ...
                        <?php foreach ($this->items as $i => $item) :
                            $item->max_ordering = 0;
                            $canEdit              = $user->authorise('core.edit', 'com_content_extended.article.' . $item->id);
                            $canCheckin           = $user->authorise('core.manage', 'com_checkin') || $item->checked_out == $userId || is_null($item->checked_out);
                            $canEditOwn           = $user->authorise('core.edit.own', 'com_content_extended.article.' . $item->id) && $item->created_by == $userId;
                            $canChange            = $user->authorise('core.edit.state', 'com_content_extended.article.' . $item->id) && $canCheckin;
                            $canExecuteTransition = $user->authorise('core.execute.transition', 'com_content_extended.article.' . $item->id);
                            $canEditCat           = $user->authorise('core.edit', 'com_content_extended.category.' . $item->catid);
                            $canEditOwnCat        = $user->authorise('core.edit.own', 'com_content_extended.category.' . $item->catid) && $item->category_uid == $userId;
                            $canEditParCat        = $user->authorise('core.edit', 'com_content_extended.category.' . $item->parent_category_id);
                            $canEditOwnParCat     = $user->authorise('core.edit.own', 'com_content_extended.category.' . $item->parent_category_id) && $item->parent_category_uid == $userId;
                                ...
                                <th scope="row" class="has-context">
                                    <div class="break-word">
                                        ...
                                        <?php if ($canEdit || $canEditOwn) : ?>
                                            <a href="<?php echo Route::_('index.php?option=com_content_extended&task=article.edit&id=' . $item->id); ?>" title="<?php echo Text::_('JACTION_EDIT'); ?> <?php echo $this->escape($item->title); ?>">
                                        ...
                                        <div class="small">
                                            <?php
                                            $ParentCatUrl = Route::_('index.php?option=com_categories&task=category.edit&id=' . $item->parent_category_id . '&extension=com_content_extended');
                                            $CurrentCatUrl = Route::_('index.php?option=com_categories&task=category.edit&id=' . $item->catid . '&extension=com_content_extended');
                    ...
                    <?php
                    if (
                        $user->authorise('core.create', 'com_content_extended')
                        && $user->authorise('core.edit', 'com_content_extended')
                        && $user->authorise('core.edit.state', 'com_content_extended')
                    ) : ?>
